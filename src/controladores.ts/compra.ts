...
export const fazerCompra = (req:Request, res:Response) => {
   ...
   const idEntificador = bancoDeDados.usuarios.find((idUsuario)=>{
        return idUsuario.id === idValido
   })
  
   
   if (!idEntificador) {
        return res.status(401).json({ mensagem: "O identificador do evento é obrigatório"})
   }
...
export const listarCompra = (req:Request, res:Response) => {
   ...
   if (!lista){
        return res.status( 403 ).json({mensagem : "O usuário não tem permissão de acessar o recurso solicitado"})
    }
